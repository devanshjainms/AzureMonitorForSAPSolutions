...
class MetricClientFactory:
    ...
    def getMetricClient(tracer: logging.Logger, 
        ...
        try:
            ...
            tracer.error("%s failed to import pyrfc module, unable to initialize NetWeaverRfcClient: %s", logTag, importEx, exc_info=True)
            raise
        except Exception as ex:
            tracer.error("%s Unexpected failure trying to create NetWeaverRfcClient: %s", logTag, ex, exc_info=True)
            raise
